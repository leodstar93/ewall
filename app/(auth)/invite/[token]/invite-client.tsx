...
export default function InvitePageClient({ token }: { token: string }) {
  ...
  const [saferBanner, setSaferBanner] = useState<{
  ...
  const handleSaferSearch = async () => {
    ...
    if (!dot) {
      setSaferBanner({
        tone: "error",
        message: "Enter a USDOT number before searching SAFER.",
      });
      return;
    }
    ...
    try {
      ...
      setSaferBanner({
        tone: "info",
        message: "Searching SAFER for your carrier details...",
      });
      ...
      const payload = (await res.json().catch(() => ({}))) as SaferLookupResponse & {
        error?: string;
      };
      ...
      setSaferBanner({
        tone: "success",
        message: "Carrier data loaded from SAFER. Review and continue.",
      });
    } catch {
