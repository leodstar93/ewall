...
export default function Form2290TaxPeriodsClient() {
  ...
  async function load() {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Could not load tax periods.");
      }
      ...
      setError(loadError instanceof Error ? loadError.message : "Could not load tax periods.");
    } finally {
      setLoading(false);
    }
  }
    ...
    () => [
      {
        ...
        render: (_value, period) => {
          ...
          return (
            <input
              value={draft.name}
              onChange={(e) => setEditing((c) => ({ ...c, [period.id]: { ...draft, name: e.target.value } }))}
              style={inputStyle}
            />
          );
        },
        ...
        render: (_value, period) => {
          ...
          return (
            <input
              ...
              onChange={(e) => setEditing((c) => ({ ...c, [period.id]: { ...draft, startDate: e.target.value } }))}
              style={inputStyle}
            />
          );
        },
        ...
        render: (_value, period) => {
          ...
          return (
            <input
              type="date"
              value={draft.endDate}
              onChange={(e) => setEditing((c) => ({ ...c, [period.id]: { ...draft, endDate: e.target.value } }))}
              style={inputStyle}
            />
          );
        },
        ...
        render: (_value, period) => {
          ...
          return (
            <input
              ...
              onChange={(e) => setEditing((c) => ({ ...c, [period.id]: { ...draft, filingDeadline: e.target.value } }))}
              style={inputStyle}
            />
          );
        },
          ...
          return (
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
              <input
                ...
                onChange={(e) => setEditing((c) => ({ ...c, [period.id]: { ...draft, isActive: e.target.checked } }))}
              />
      ...
      },
      {
        ...
        render: (_value, period) => (
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
            <button type="button" onClick={() => setSelectedPeriodId(period.id)} className={tableStyles.btn}>
              Rate Table
            </button>
            <button
              type="button"
            ...
            <button
              ...
              style={{
                height: 30, padding: "0 12px", border: "1px solid #fecaca", borderRadius: 6,
                fontSize: 12, cursor: "pointer", background: "transparent", color: "#b91c1c",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {loading ? (
        <div className={tableStyles.card}><div style={{ padding: 20, fontSize: 13, color: "#aaa" }}>Loading tax periods...</div></div>
      ) : (
        <>
      ...
      {selectedPeriodId
        ? createPortal(
            <div
              ...
              onClick={(e) => { if (e.target === e.currentTarget) setSelectedPeriodId(null); }}
            >
              <div style={{
                background: "var(--bg, #fff)", borderRadius: 12, width: "100%", maxWidth: 960,
                boxShadow: "0 24px 64px rgba(0,0,0,0.18)", display: "flex", flexDirection: "column", overflow: "hidden",
              }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: "1px solid var(--brl)", flexShrink: 0 }}>
                  ...
                  <button
                    ...
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: 22, lineHeight: 1, color: "#888", padding: "0 4px" }}
                    aria-label="Close"
                  >
                    ×
                  </button>
