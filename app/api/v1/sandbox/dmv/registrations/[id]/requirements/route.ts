  ...
  try {
    ...
    return Response.json({
      ...
      availableDocuments: await ctx.db.document.findMany({
        where: {
          OR: [{ userId: registration.userId }, { userId: ctx.actorUserId }],
        },
...
export async function DELETE(
  ...
  try {
    ...
    await createSandboxAuditFromContext(ctx, {
      action: "sandbox.dmv.staff.document.unlink",
...
export async function PATCH(
  ...
  try {
    ...
    if (typeof body.documentId === "string" && body.documentId.trim()) {
      ...
      await createSandboxAuditFromContext(ctx, {
        action: "sandbox.dmv.staff.document.attach",
        entityType: "DmvRegistration",
        entityId: id,
        metadataJson: {
          documentId: body.documentId,
    ...
    await createSandboxAuditFromContext(ctx, {
      action: "sandbox.dmv.staff.requirement.update",
      entityType: "DmvRegistration",
      entityId: id,
      metadataJson: {
        code,
