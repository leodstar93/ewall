...
export async function GET() {
  ...
  try {
    const truckWhere = guard.isAdmin ? {} : { userId: guard.session.user.id };
    const registrationWhere = guard.isAdmin ? {} : { userId: guard.session.user.id };
    ...
    const [trucks, registrations, renewals] = await Promise.all([
      ...
      prisma.dmvRenewal.findMany({
        where: guard.isAdmin ? {} : { registration: { userId: guard.session.user.id } },
        include: {
          registration: {
            select: {
              id: true,
    ...
    const highRiskRenewals = renewals.filter((renewal) =>
      calculateRenewalRisk({ dueDate: renewal.dueDate, now }) === "HIGH_RISK",
    ).length;
    ...
    console.error("Failed to load DMV dashboard", error);
    return Response.json({ error: "Failed to load DMV dashboard" }, { status: 500 });
  }
}
