...
export async function POST(
  ...
  try {
    ...
    const result = await FilingWorkflowService.refreshDerivedState({
      filingId: id,
      actorUserId: userId,
    });

    return Response.json({ rebuilt: true, result });
  } catch (error) {
    return handleIftaAutomationError(error, "Failed to rebuild IFTA filing.");
  }
}
