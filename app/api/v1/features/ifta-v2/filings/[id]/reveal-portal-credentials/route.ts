...
import { revealPortalCredentialsForFiling } from "@/services/ifta-automation/ifta-access.service";
...
export async function POST(
  ...
  try {
    ...
    const payload = await revealPortalCredentialsForFiling({
      filingId: id,
      actorUserId: userId,
    });
    ...
    return handleIftaAutomationError(
      error,
      "Failed to reveal IFTA portal credentials.",
    );
  }
}
